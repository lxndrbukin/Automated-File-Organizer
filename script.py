...
if __name__ == "__main__":
    ...
    parser.add_argument("--verbose", action="store_true", help="Log with additional details per file")
    parser.add_argument("--config", type=str, help="Custom config file path")
    args = parser.parse_args()
    if args.src:
        custom_src_dir = Path(args.src)
        if not Path.exists(custom_src_dir):
            print(f"'{custom_src_dir}' doesn't exist")
            exit(1)
        src_dir = custom_src_dir
    if args.config:
        custom_config = Path(args.config)
        if not Path.exists(custom_config):
            print("Config path doesn't exist")
            exit(1)
        config = load_config(custom_config)
    run_script(dry_run=args.dry_run, verbose=args.verbose)